...
import { createLead, getLeadByChatId, updateLeadThreadId, updateLeadMainThreadId } from '../dao/leadDAO.js';
...
export const mainBotMsg = async (prompt, chatId) => {
    try {
        let lead = await getLeadByChatId(chatId);
        let threadId = lead ? lead.threadId : null;

        let response = await sendMessage(prompt, threadId, config.MAIN_ASSISTANT_ID);
        if (!threadId && response.threadId) {
            // await createLead(chatId, response.threadId);
            await updateLeadMainThreadId(chatId, response.threadId);
        }
        return response.response;
    } catch (error) {
        console.log(error);
